...
function IngredientRow({ ingredient, index, onAmountChange, onRemove }: {
    ingredient: Ingredient;
    index: number;
    onAmountChange: (index: number, amount: number) => void;
    onRemove: (index: number) => void;
}) {
    const [localAmount, setLocalAmount] = useState(String(ingredient.amountG));
    const cal = Math.round((ingredient.caloriesPer100g * ingredient.amountG) / 100);

    return (
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, py: 1.5, flexWrap: { xs: 'wrap', sm: 'nowrap' } }}>
            ...
            <TextField
                ...
                onBlur={() => {
                    ...
                    if (num >= 1) {
                        ...
                        setLocalAmount(String(ingredient.amountG));
                    }
                }}
                size="small"
